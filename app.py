...
def process_image(image):
    ...
    temp_file = r"./Images/processed_image.jpg"
    ...
    temp_json_path = r"./static/extracted_data.json"
...
def extract_date(extracted_text):
    ...
    for label, preceding_patterns in preceding_labels.items():
        if label not in extracted_dates:
            for preceding_pattern in preceding_patterns:
                pattern = re.escape(preceding_pattern) + r'\s(' + '|'.join(date_patterns) + ')'
                match = re.search(pattern, extracted_text, re.IGNORECASE)
                if match:
                    extracted_dates[label] = match.group(1)
                    return match.group(1)  # Return the extracted date directly
